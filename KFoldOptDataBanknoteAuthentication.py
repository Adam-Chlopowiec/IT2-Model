...
from Settings.SettingsDataBanknoteAuthentication import Settings
...
def main():
    best_params = pd.DataFrame(
        [['Gaussian Progressive', 11, 'Mean'], ['Gaussian Equal', 11, 'Mean'], ['Gaussian Equal', 9, 'Mean'],
         ['Gaussian Equal', 11, 'Center']],
    ...
    for _, data in best_params.iterrows():
        ...
        results.to_csv(r'Test - Datasets\ResultsIT2\DataBanknoteAuthenticationSigmaCenterKFoldGrid' + str(i) + '.csv', index=None)
        i += 1

    results.to_csv(r'Test - Datasets\ResultsIT2\DataBanknoteAuthenticationSigmaCenterKFoldGrid.csv', index=None)
