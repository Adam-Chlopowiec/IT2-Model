...
class Fuzzify(object):
    ...
    def fuzzifyFeatures(self, train_features_df = -1, sigma_mean_params = -1, sigma_offset = 0, default_fuzzifier = -1):
        if isinstance(default_fuzzifier, (int, np.integer)):
    ...
    def workerKFold(self, settings, train_features_df, sigma_mean_params, sigma_offset=0, fuzzifier = -1):
        self.settings = settings
        changed_decisions, features_number_after_reduct, implicants_number, features, decision_table_with_reduct, reductor = self.fuzzifyFeatures(train_features_df, sigma_mean_params, sigma_offset, fuzzifier)
        return changed_decisions, features_number_after_reduct, implicants_number, features, decision_table_with_reduct, reductor