...
class Fuzzifier(object):
    ...
    def __init__(self, settings, d_results):
        ...
        for i in range(0, self.settings.feature_numbers):
            ...
            self.features.append(ctrl.Antecedent(self.x_range, feature_label))
    ...
    def gaussianFunction(self, x, mean, sigma, center_l, center_r, bottom_l, bottom_r, sigma_offset=0):
        y = np.zeros(len(x))
        ...
        if (center_l == -1) and (center_r == -1):
            ...
            y[idx] = 1 - np.exp(-((x[idx] - bottom_r) ** 2.) / (2 * (sigma - sigma_offset) ** 2.))
        elif center_r == -1:
            idx = (x >= bottom_l) & (x < center_l)
            y[idx] = 1 - np.exp(-((x[idx] - bottom_l) ** 2.) / (2 * (sigma - sigma_offset) ** 2.))
    ...
    def gaussianValue(self, x, mean, sigma, center_l, center_r, bottom_l, bottom_r, sigma_offset=0):

        if (center_l == -1) and (center_r == -1):
            if (x < bottom_r) and (x >= bottom_l):
                y = np.exp(-((x - mean) ** 2.) / (2 * (sigma + sigma_offset) ** 2.))
            else:
                y = 0
        elif center_l == -1:
            if (x < center_r) and (x >= bottom_l):
                ...
                y = 1 - np.exp(-((x - bottom_r) ** 2.) / (2 * (sigma - sigma_offset) ** 2.))
            else:
                y = 0
        elif center_r == -1:
            if (x < bottom_r) and (x >= center_l):
                ...
                y = 1 - np.exp(-((x - bottom_l) ** 2.) / (2 * (sigma - sigma_offset) ** 2.))
            else:
                y = 0
        else:
    ...
    def generateGausses3(self, x_range: np.ndarray, mean: float, sigma: float, sigma_offset: float = 0) -> tuple:
        verylow = self.gaussLeft(self.x_range, mean, sigma - sigma_offset)
        middle = self.gaussianFunction(self.x_range, mean, sigma, -1, -1, 0, 1.01, sigma_offset)
        veryhigh = self.gaussRight(self.x_range, mean, sigma - sigma_offset)
        return verylow, middle, veryhigh

    def generateGausses5(self, x_range: np.ndarray, mean: float, sigma_offset: float = 0) -> tuple:
        self.cr_low, sigma = self.calculateSigma(mean * (1 / 2), mean)
        verylow = self.gaussLeft(self.x_range, mean * (1 / 2), sigma - sigma_offset)
        low = self.gaussianFunction(self.x_range, mean * (1 / 2), sigma, -1, self.cr_low, 0, mean, sigma_offset)

        self.cr_middle, _ = self.calculateSigma(mean, mean * (3 / 2))
        middle = self.gaussianFunction(self.x_range, mean, sigma, self.cr_low, self.cr_middle, mean * (1 / 2),
                                       mean * (3 / 2), sigma_offset)

        high = self.gaussianFunction(self.x_range, mean * (3 / 2), sigma, self.cr_middle, -1, mean, 1.01, sigma_offset)
        veryhigh = self.gaussRight(self.x_range, mean * (3 / 2), sigma - sigma_offset)
        return sigma, verylow, low, middle, high, veryhigh

    def generateGausses7(self, x_range: np.ndarray, mean: float, sigma_offset: float = 0) -> tuple:
        self.cr_low, sigma = self.calculateSigma(mean * (1 / 3), mean * (2 / 3))
        verylow = self.gaussLeft(self.x_range, mean * (1 / 3), sigma - sigma_offset)
        low = self.gaussianFunction(self.x_range, mean * (1 / 3), sigma, -1, self.cr_low, 0, mean * (2 / 3),
                                    sigma_offset)

        self.cr_middlelow, sigma = self.calculateSigma(mean * (2 / 3), mean)
        middlelow = self.gaussianFunction(self.x_range, mean * (2 / 3), sigma, self.cr_low, self.cr_middlelow,
                                          mean * (1 / 3), mean, sigma_offset)

        self.cr_middle, sigma = self.calculateSigma(mean, mean * (4 / 3))
        middle = self.gaussianFunction(self.x_range, mean, sigma, self.cr_middlelow, self.cr_middle, mean * (2 / 3),
                                       mean * (4 / 3), sigma_offset)

        self.cr_middlehigh, sigma = self.calculateSigma(mean * (4 / 3), mean * (5 / 3))
        middlehigh = self.gaussianFunction(self.x_range, mean * (4 / 3), sigma, self.cr_middle, self.cr_middlehigh,
                                           mean, mean * (5 / 3), sigma_offset)

        high = self.gaussianFunction(self.x_range, mean * (5 / 3), sigma, self.cr_middlehigh, -1, mean * (4 / 3), 1.01,
                                     sigma_offset)
    ...
    def generateGausses9(self, x_range: np.ndarray, mean: float, sigma_offset: float = 0) -> tuple:
        self.cr_low, sigma = self.calculateSigma(mean * (1 / 4), mean * (2 / 4))
        verylow = self.gaussLeft(self.x_range, mean * (1 / 4), sigma - sigma_offset)
        low = self.gaussianFunction(self.x_range, mean * (1 / 4), sigma, -1, self.cr_low, 0, mean * (2 / 4),
                                    sigma_offset)

        self.cr_middlelowminus, sigma = self.calculateSigma(mean * (2 / 4), mean * (3 / 4))
        middlelowminus = self.gaussianFunction(self.x_range, mean * (2 / 4), sigma, self.cr_low, self.cr_middlelowminus,
                                               mean * (1 / 4), mean * (3 / 4), sigma_offset)

        self.cr_middlelow, sigma = self.calculateSigma(mean * (3 / 4), mean)
        middlelow = self.gaussianFunction(self.x_range, mean * (3 / 4), sigma, self.cr_middlelowminus,
                                          self.cr_middlelow, mean * (2 / 4), mean, sigma_offset)

        self.cr_middle, sigma = self.calculateSigma(mean, mean * (5 / 4))
        middle = self.gaussianFunction(self.x_range, mean, sigma, self.cr_middlelow, self.cr_middle, mean * (3 / 4),
                                       mean * (5 / 4), sigma_offset)

        self.cr_middlehigh, sigma = self.calculateSigma(mean * (5 / 4), mean * (6 / 4))
        middlehigh = self.gaussianFunction(self.x_range, mean * (5 / 4), sigma, self.cr_middle, self.cr_middlehigh,
                                           mean, mean * (6 / 4), sigma_offset)

        self.cr_middlehighplus, sigma = self.calculateSigma(mean * (6 / 4), mean * (7 / 4))
        middlehighplus = self.gaussianFunction(self.x_range, mean * (6 / 4), sigma, self.cr_middlehigh,
                                               self.cr_middlehighplus, mean * (5 / 4), mean * (7 / 4), sigma_offset)

        high = self.gaussianFunction(self.x_range, mean * (7 / 4), sigma, self.cr_middlehighplus, -1, mean * (6 / 4),
                                     1.01, sigma_offset)
    ...
    def generateGausses11(self, x_range: np.ndarray, mean: float, sigma_offset: float = 0) -> tuple:
        self.cr_low, sigma = self.calculateSigma(mean * (1 / 5), mean * (2 / 5))
        verylow = self.gaussLeft(self.x_range, mean * (1 / 5), sigma - sigma_offset)
        low = self.gaussianFunction(self.x_range, mean * (1 / 5), sigma, -1, self.cr_low, 0, mean * (2 / 5),
                                    sigma_offset)

        self.cr_middlelowminus, sigma = self.calculateSigma(mean * (2 / 5), mean * (3 / 5))
        middlelowminus = self.gaussianFunction(self.x_range, mean * (2 / 5), sigma, self.cr_low, self.cr_middlelowminus,
                                               mean * (1 / 5), mean * (3 / 5), sigma_offset)

        self.cr_middlelow, sigma = self.calculateSigma(mean * (3 / 5), mean * (4 / 5))
        middlelow = self.gaussianFunction(self.x_range, mean * (3 / 5), sigma, self.cr_middlelowminus,
                                          self.cr_middlelow, mean * (2 / 5), mean * (4 / 5), sigma_offset)

        self.cr_middlelowplus, sigma = self.calculateSigma(mean * (4 / 5), mean * (5 / 5))
        middlelowplus = self.gaussianFunction(self.x_range, mean * (4 / 5), sigma, self.cr_middlelow,
                                              self.cr_middlelowplus, mean * (3 / 5), mean * (5 / 5), sigma_offset)

        self.cr_middle, sigma = self.calculateSigma(mean, mean * (6 / 5))
        middle = self.gaussianFunction(self.x_range, mean, sigma, self.cr_middlelowplus, self.cr_middle, mean * (4 / 5),
                                       mean * (6 / 5), sigma_offset)

        self.cr_middlehighminus, sigma = self.calculateSigma(mean * (6 / 5), mean * (7 / 5))
        middlehighminus = self.gaussianFunction(self.x_range, mean * (6 / 5), sigma, self.cr_middle,
                                                self.cr_middlehighminus, mean * (5 / 5), mean * (7 / 5), sigma_offset)

        self.cr_middlehigh, sigma = self.calculateSigma(mean * (7 / 5), mean * (8 / 5))
        middlehigh = self.gaussianFunction(self.x_range, mean * (7 / 5), sigma, self.cr_middlehighminus,
                                           self.cr_middlehigh, mean * (6 / 5), mean * (8 / 5), sigma_offset)

        self.cr_middlehighplus, sigma = self.calculateSigma(mean * (8 / 5), mean * (9 / 5))
        middlehighplus = self.gaussianFunction(self.x_range, mean * (8 / 5), sigma, self.cr_middlehigh,
                                               self.cr_middlehighplus, mean * (7 / 5), mean * (9 / 5), sigma_offset)

        high = self.gaussianFunction(self.x_range, mean * (9 / 5), sigma, self.cr_middlehighplus, -1, mean * (8 / 5),
                                     1.01, sigma_offset)
    ...
    def gaussianFunValues5(self, x: float, mean: float, sigma: float, sigma_offset: float = 0) -> tuple:
        ...
        middle_value = self.gaussianValue(x, mean, sigma + sigma_offset, self.cr_low, self.cr_middle, mean * (1 / 2),
                                          mean * (3 / 2))
    ...
    def gaussianFunValues7(self, x: float, mean: float, sigma: float, sigma_offset: float = 0) -> tuple:
        ...
        middlelow_value = self.gaussianValue(x, mean * (2 / 3), sigma + sigma_offset, self.cr_low, self.cr_middlelow,
                                             mean * (1 / 3), mean)
        middle_value = self.gaussianValue(x, mean, sigma + sigma_offset, self.cr_middlelow, self.cr_middle,
                                          mean * (2 / 3), mean * (4 / 3))
        middlehigh_value = self.gaussianValue(x, mean * (4 / 3), sigma + sigma_offset, self.cr_middle,
                                              self.cr_middlehigh, mean, mean * (5 / 3))
        high_value = self.gaussianValue(x, mean * (5 / 3), sigma + sigma_offset, self.cr_middlehigh, -1, mean * (4 / 3),
                                        1.01)
    ...
    def gaussianFunValues9(self, x: float, mean: float, sigma: float, sigma_offset: float = 0) -> tuple:
        ...
        middlelowminus_value = self.gaussianValue(x, mean * (2 / 4), sigma + sigma_offset, self.cr_low,
                                                  self.cr_middlelowminus, mean * (1 / 4), mean * (3 / 4))
        middlelow_value = self.gaussianValue(x, mean * (3 / 4), sigma + sigma_offset, self.cr_middlelowminus,
                                             self.cr_middlelow, mean * (2 / 4), mean)
        middle_value = self.gaussianValue(x, mean, sigma + sigma_offset, self.cr_middlelow, self.cr_middle,
                                          mean * (3 / 4), mean * (5 / 4))
        middlehigh_value = self.gaussianValue(x, mean * (5 / 4), sigma + sigma_offset, self.cr_middle,
                                              self.cr_middlehigh, mean, mean * (6 / 4))
        middlehighplus_value = self.gaussianValue(x, mean * (6 / 4), sigma + sigma_offset, self.cr_middlehigh,
                                                  self.cr_middlehighplus, mean * (5 / 4), mean * (7 / 4))
        high_value = self.gaussianValue(x, mean * (7 / 4), sigma + sigma_offset, self.cr_middlehighplus, -1,
                                        mean * (6 / 4), 1.01)
    ...
    def gaussianFunValues11(self, x: float, mean: float, sigma: float, sigma_offset: float = 0) -> tuple:
        ...
        middlelowminus_value = self.gaussianValue(x, mean * (2 / 5), sigma + sigma_offset, self.cr_low,
                                                  self.cr_middlelowminus, mean * (1 / 5), mean * (3 / 5))
        middlelow_value = self.gaussianValue(x, mean * (3 / 5), sigma + sigma_offset, self.cr_middlelowminus,
                                             self.cr_middlelow, mean * (2 / 5), mean * (4 / 5))
        middlelowplus_value = self.gaussianValue(x, mean * (4 / 5), sigma + sigma_offset, self.cr_middlelow,
                                                 self.cr_middlelowplus, mean * (3 / 5), mean * (5 / 5))
        middle_value = self.gaussianValue(x, mean, sigma + sigma_offset, self.cr_middlelowplus, self.cr_middle,
                                          mean * (4 / 5), mean * (6 / 5))
        middlehighminus_value = self.gaussianValue(x, mean * (6 / 5), sigma + sigma_offset, self.cr_middle,
                                                   self.cr_middlehighminus, mean * (5 / 5), mean * (7 / 5))
        middlehigh_value = self.gaussianValue(x, mean * (7 / 5), sigma + sigma_offset, self.cr_middlehighminus,
                                              self.cr_middlehigh, mean * (6 / 5), mean * (8 / 5))
        middlehighplus_value = self.gaussianValue(x, mean * (8 / 5), sigma + sigma_offset, self.cr_middlehigh,
                                                  self.cr_middlehighplus, mean * (7 / 5), mean * (9 / 5))
        high_value = self.gaussianValue(x, mean * (9 / 5), sigma + sigma_offset, self.cr_middlehighplus, -1,
                                        mean * (8 / 5), 1.01)
    ...
    def numbersToRowSets(self, idx, values, mean, sigma_offset=0):
        ...
        if self.settings.gausses == 3:
            ...
            names = (
            self.settings.verylow, self.settings.low, self.settings.middle, self.settings.high, self.settings.veryhigh)
            for lower, upper, name in zip(lower_functions, upper_functions, names):
                self.features[idx][name] = (lower, upper)
            ...
            names = (self.settings.verylow, self.settings.low, self.settings.middlelow, self.settings.middle,
                     self.settings.middlehigh, self.settings.high, self.settings.veryhigh)
            for lower, upper, name in zip(lower_functions, upper_functions, names):
                self.features[idx][name] = (lower, upper)
            ...
            names = (self.settings.verylow, self.settings.low, self.settings.middlelowminus, self.settings.middlelow,
                     self.settings.middle, self.settings.middlehigh, self.settings.middlehighplus, self.settings.high,
                     self.settings.veryhigh)
            for lower, upper, name in zip(lower_functions, upper_functions, names):
                self.features[idx][name] = (lower, upper)
            ...
            names = (self.settings.verylow, self.settings.low, self.settings.middlelowminus, self.settings.middlelow,
                     self.settings.middlelowplus, self.settings.middle, self.settings.middlehighminus,
        ...
        for x in values:
            ...
            if self.settings.gausses == 5:
                verylow_value, low_value, middle_value, high_value, veryhigh_value = self.gaussianFunValues5(x, mean,
                                                                                                             sigma)

            elif self.settings.gausses == 7:
                verylow_value, low_value, middlelow_value, middle_value, middlehigh_value, high_value, veryhigh_value = self.gaussianFunValues7(
                    x, mean, sigma)
                ...
                middlehighminus_value, middlehigh_value, middlehighplus_value, high_value, veryhigh_value = self.gaussianFunValues11(
                    x, mean, sigma)

            max_value = max(
                [verylow_value, low_value, middlelowminus_value, middlelow_value, middlelowplus_value, middle_value,
                 middlehighminus_value, middlehigh_value, middlehighplus_value, high_value, veryhigh_value])
