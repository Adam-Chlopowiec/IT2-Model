...
from Settings.SettingsIonosphere import Settings
...
def main():
    best_params = pd.DataFrame(
        [['Gaussian Equal', 3, 'Mean'], ['Gaussian Progressive', 3, 'Mean'], ['Gaussian Equal', 5, 'Mean'],
         ['Gaussian Equal', 7, 'Mean']],
    ...
    for _, data in best_params.iterrows():
        ...
        results.to_csv(r'Test - Datasets\ResultsIT2\IonosphereSigmaCenterKFoldGrid' + str(i) + '.csv', index=None)
        i += 1

    results.to_csv(r'Test - Datasets\ResultsIT2\IonosphereSigmaCenterKFoldGrid.csv', index=None)
